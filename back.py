...
Please format the response with clear section headers and bullet points for readability."""
    ...
    def upload_documents(
        ...
        try:
            ...
            if file_details:
                ...
                processing_time = int((time.time() - start_time) * 1000)  # Calculate time in milliseconds
                metrics_query = f"""
                INSERT INTO ANALYTICS_METRICS (
                    METRIC_ID,
                    TIMESTAMP,
    ...
    def semantic_search_with_llm(
        ...
        try:
            ...
            if filename:
                escaped_filename = filename.replace("'", "''")
                filename_condition = f"WHERE FILENAME = '{escaped_filename}' AND"
            else:
                filename_condition = "WHERE"
            # Optimize the search query to reduce data transfer
            # Semantic search query remains the same
            ...
            WITH search_results AS (
                SELECT 
                    ...
                    FILENAME,
                    CASE 
                        WHEN LENGTH(CONTENT) > {max_tokens_per_context * 4} THEN 
                            LEFT(CONTENT, {max_tokens_per_context * 4})
                        ELSE CONTENT 
                    END as CONTENT,
                    METADATA,
            ...
            print(f"Debug - Results before formatting: {len(results)}")  
            ...
            for idx, row in enumerate(results):
                ...
                #print(f"Available fields: {row.keys()}" if hasattr(row, 'keys') else f"Attributes: {dir(row)}")
                    try:
                        content = row['CONTENT'].strip() if row['CONTENT'] is not None else ""
                        
                        processed_results.append({
                            'DOC_ID': row['DOC_ID'],
                            'FILENAME': row['FILENAME'],
                            'CONTENT': content[:300] + "..." if len(content) > 300 else content,
                            'PAGE_NUMBER': int(row['PAGE_NUMBER']) if row['PAGE_NUMBER'] is not None else 1,
                            'SIMILARITY_SCORE': float(row['SIMILARITY_SCORE'])
                        })
                        print(f"Debug - Results after formatting: {len(processed_results)}")  
                        print(f"Debug - Successfully processed result for {row['FILENAME']}")
                    except Exception as e:
            ...
            SELECT SNOWFLAKE.CORTEX.COMPLETE(
                'mistral-large2',
                ARRAY_CONSTRUCT(
                    OBJECT_CONSTRUCT('role', 'system', 'content', '{style_instr}\n{format_instr}'),
                    OBJECT_CONSTRUCT('role', 'user', 'content', 'Question: {escaped_query}\n\nContext:\n{context.replace("'", "''")}')
                ),
                OBJECT_CONSTRUCT(
